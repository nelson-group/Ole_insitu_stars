...
def insitu_catalog(run, stype, start_snap):
    ...
    num_tracers = star_formation_distances.shape[0]
    f.close()
    ...
    assert isfile('/vera/ptmp/gc/olwitt/' + stype + f'/TNG50-{run}/parent_indices_{start_snap}.hdf5')
    ...
    return insitu_cat, final_offsets

run = int(sys.argv[1])
stype = 'insitu'
start_snap = 99

cat, offsets = insitu_catalog(run, stype, start_snap)
